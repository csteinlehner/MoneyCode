...
- **`sw.js`** — service worker for offline caching (cache name: `girocode-v1`). When adding/removing files, update the `FILES` array here.
...
Deployed to GitHub Pages via `.github/workflows/pages.yml` on push to `main`. The workflow uploads the entire repo root as the site artifact.
